...
// bool返回为true为有记录 执行Update
// bool返回为false标识没有记录 该执行Insert

// err只有在type没有设置 "save" tag的时候返回
// todo
func IsExist(sqlDb TDManager, args interface{}) (bool,error) {

	temp := reflect.TypeOf(args).Elem()
	for i := 0; i < temp.NumField(); i++ {
		if values,ok :=temp.Field(i).Tag.Lookup("save"); ok{

			if reflect.ValueOf(args).Field(i).String() == ""{
				return false,nil
			}
			value := strings.Split(values, ",")
			table := value[0]
			where := map[string]interface{}{
				value[1] : reflect.ValueOf(args).Field(i).String(),
			}
			what := []string{"*"}
			cond, vals, _ := builder.BuildSelect(table, where, what)
			_, err := sqlDb.Query(cond, vals)
	...
	return false,errors.New("请设置字段tag")
}

func DBQuery(trace *TraceContext, sqlDb TDManager, query string, args ...interface{}) (*sql.Rows, error) {
...
func DBExec(trace *TraceContext, sqlDb TDManager, sql string, args ...interface{}) (sql.Result, error) {
	startExecTime := time.Now()
